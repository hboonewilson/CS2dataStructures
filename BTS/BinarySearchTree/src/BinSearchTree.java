...
public class BinSearchTree<E extends Comparable<E>>
  ...
  public void add(E e) {
    if ( isEmpty() )
      ...
      int comparison = e.compareTo(pos.element());
      BinTreeNode<E> node = new BinTreeNode<>(e,pos , null , null);
      if ( comparison == 0 ) // found e in tree!
        return;
      else if ( comparison < 0 ){ pos.setLeft(node); }

      else{ pos.setRight(node); } // comparison > 0
      // code here
    ...
    if ( pos.element().equals(e) ) {
      ...
      if ( pos.left() == null && pos.right() == null ) {
        if ( pos.isRoot() ) {
          ...
          // code here (case 1) 2 lines
          int comparison = p.element().compareTo(pos.element());
          if (comparison > 0) {p.setLeft(null);}
          else {p.setRight(null);}
        }
      }
      else if ( pos.left() == null && pos.right() != null ) {
        if (  pos.isRoot() ) {
          pos.left().setParent(pos.right());
          pos.setParent(null);
          // pos is root
          // code here (2 lines)
        ...
        if ( pos.isRoot() ) {
          ...
          if ( pos == pos.parent().left() ){ pos.parent().setLeft(sub); }
          // code here
          else {pos.parent().setRight(sub);}
          // code here
        }
      }
      return true;
