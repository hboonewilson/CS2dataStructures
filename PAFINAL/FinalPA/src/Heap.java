...
public class Heap<E> {
    ...
    public boolean isLeaf(Node<E> node)
    {
        ...
        else { return node.getLeftChild() == null && node.getRightChild() == null; }
    ...
    private void heapify(Node<E> startNode)
    {
        ...
        {
           E left = startNode.getLeftChild().getElement();
           E right = startNode.getRightChild().getElement();
            //Case 1: your code here... (hint: you need to decide which child node is larger and recurse over that node)
            if (comp.compare(left, right) > 0){
                Node<E> nodeToSwap = startNode.getLeftChild();
                //recursive left
                swap(nodeToSwap, startNode);
                heapify(startNode);
            }
            else{
                Node<E> nodeToSwap = startNode.getRightChild();
                swap(nodeToSwap, startNode);
                heapify(startNode);
                //recursive right
            }
        ...
        else if(startNode.getRightChild() != null && comp.compare(startNode.getElement(), startNode.getRightChild().getElement()) < 0)
        {
            ...
            swap(startNode.getRightChild(), startNode);
            return;
        }
        else if(startNode.getLeftChild() != null && comp.compare(startNode.getElement(), startNode.getLeftChild().getElement()) < 0)
        {
            ...
            swap(startNode.getLeftChild(), startNode);
            return;
        }
    }
